...
if [[ -d $folder ]]; then 
    ...
    git subtree pull $remote main --prefix=$folder -m "Update from '$remote' at '$remote_latest_commit'"
else 
    # add the subtree
    git subtree add  $remote main --prefix=$folder -m "Add '$folder'/ from '$remote' at '$remote_latest_commit'"
fi
